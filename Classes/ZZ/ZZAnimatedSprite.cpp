...
AnimatedSprite::~AnimatedSprite(void)
{
	...
	std::map<std::string, Animation*>::iterator itr = m_animations.begin();
...
bool AnimatedSprite::init(  AnimationLogic* animLogic, JsonValueRef animSprite )
{
	...
	*/
	...
	Texture2D* texture = TextureCache::sharedTextureCache()->addImage( path.c_str() );
	//setTexture(texture);
	...
	Rect area = Rect(0,0, uniformFrameSize, uniformFrameSize);
	Sprite::initWithTexture(texture, area);
	...
	for( int animSetIdx=0; animSetIdx< numAnims; animSetIdx++)
	{
		...
		cocos2d::Vector<SpriteFrame*> frameList;
		...
		for( int currFrame=0; currFrame<numFrames; currFrame++)
		{
			...
			Rect fRect = Rect( uniformFrameSize*col, uniformFrameSize*row, uniformFrameSize, uniformFrameSize);
			...
			SpriteFrame* sFrame = SpriteFrame::createWithTexture(texture, fRect, false, Point(0,0), fRect.size);
			
			//sFrameList->addObject(sFrame);
			frameList.pushBack(sFrame);
		
		}
		 

		//Animation::createWithSpriteFrames(const cocos2d::Vector<cocos2d::SpriteFrame*>& arrayofspriteframeNames
		Animation* animation = Animation::createWithSpriteFrames(frameList, secondsPerFrame);
...
bool AnimatedSprite::setAnimState( std::string stateName )
{
	...
	if(!reuseAnim) {
		...
		//run current anim
		m_animAction = Animate::create( m_currAnim );
	
		m_animActionSequence = Sequence::createWithTwoActions( m_animAction, CallFunc::create(this, callfunc_selector(AnimatedSprite::onAnimStateEnd)) );
