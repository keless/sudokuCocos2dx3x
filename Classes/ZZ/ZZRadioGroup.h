...
public:
	...
	virtual void handleRadioGroupEvt( Ref* e ) = 0;
...
public:
	BaseRadioGroupLayer()
	{
		m_highlight = LayerColor::create(Color4B(255,255,255,255));
		addChild(m_highlight);
		m_highlight->setVisible(false);
	}
	//virtual ~BaseRadioGroupLayer();

	virtual void setContentSize( const Size& area )
	{
		CCNode::setContentSize(area);
		m_highlight->setContentSize(area);
	}

	virtual void handleRadioGroupEvt( Ref* e )
	{
		m_highlight->setVisible(false); 
	}
