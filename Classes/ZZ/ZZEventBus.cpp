...
void EventBus::addListener(std::string evtName, Ref* listener, SEL_CallFuncO callback  )
{
	callbackPair cbp;
...
void EventBus::remListener(std::string evtName, Ref* listener, SEL_CallFuncO callback  )
{
	if( m_listeners.count( evtName ) == 0 ) {
		return;
	}
...
void EventBus::dispatch(std::string evtName, Ref* evtObject )
{
	std::vector<callbackPair> & listeners = m_listeners[evtName];
	for( int i=0; i< (int) listeners.size(); i++) 
	{
