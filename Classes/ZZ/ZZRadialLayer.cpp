...
bool RadialLayer::init( float radius , float tierRadius  )
{
	...
	m_bg->drawDot( Point(0.0f,0.0f), 150, Color4F(0.7f,0.7f,0.7f,0.7f));
	addChild(m_bg);
...
void RadialLayer::setCenterNode( Node* node ) 
{
	if( m_center != NULL ) {
	...
	m_center->setAnchorPoint(Point(0.5,0.5));
...
void RadialLayer::addItem( Node* node, std::string evt, std::string bus )
{
...
void RadialLayer::addItemWithJson( Node* item, std::string evt, JsonValueRef json, std::string bus )
{
